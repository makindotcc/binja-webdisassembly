...
async function Ey() {
    ...
    const wasmBytes = await (await fetch("/.well-known/vercel/security/static/challenge.v2.wasm")).arrayBuffer();
    const result = await WebAssembly.instantiate(wasmBytes, EO.importObject);

    EO.run(result.instance);
    return {

...
var E6 = class {
    ...
    _makeFuncWrapper(Ek) {
        console.log('[DBG] _makeFuncWrapper called with id:', Ek);
        let EV = this;
        return function () {
            console.log('[DBG] wrapper invoked, id:', Ek);
            let EO = {
                id: Ek,
                this: this,
                args: arguments
...
async function Ey() {
    const wasmModule = require("./challenge_wasm.js");

    let EO = new E6();
    EO.importObject.gojs["syscall/js.finalizeRef"] = () => null;

    // Wire E6 runtime imports into the decompiled module's import stubs
    const gojs = EO.importObject.gojs;
    ...
    wasmModule.imports.gojs_syscall_js_valueCall = gojs["syscall/js.valueCall"];
    wasmModule.imports.gojs_syscall_js_valueInvoke = gojs["syscall/js.valueInvoke"];
    wasmModule.imports.gojs_syscall_js_valueNew = gojs["syscall/js.valueNew"];
    wasmModule.imports.gojs_syscall_js_valueLength = gojs["syscall/js.valueLength"];
    wasmModule.imports.gojs_syscall_js_valuePrepareString = gojs["syscall/js.valuePrepareString"];
    wasmModule.imports.gojs_syscall_js_valueLoadString = gojs["syscall/js.valueLoadString"];
    wasmModule.imports.wasi_snapshot_preview1_fd_write = wasi.fd_write;
    wasmModule.imports.wasi_snapshot_preview1_random_get = wasi.random_get;

    // Create a fake WASM instance that the E6 Go runtime can use
    let EZ = {
        exports: {
            memory: wasmModule.memory,
            _start: wasmModule._start,
            resume: wasmModule.resume,
            go_scheduler: wasmModule.go_scheduler,
        }
    };
...
async function Ej(Ek) {
    ...
    console.log("[DBG] Ej: calling Ey()...");
    try { await Ey(); console.log("[DBG] Ey() resolved OK"); } catch(e) { console.error("[DBG] Ey() threw:", e); throw e; }
    let EO;
    try {
        console.log("[DBG] Ej: calling Solve...");
        let EZ = await EV.niWqu(Solve, Ek.token);
        console.log("[DBG] Solve raw result:", EZ);
        EO = JSON.parse(EZ);
        console.log("[DBG] Solve parsed:", JSON.stringify(EO));
        let Eq = EO.solution;
        console.log("[DBG] solution:", Eq, "version:", Ek.version);
